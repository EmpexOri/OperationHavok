...
func _perform_explode():
	...
	for i in range(grenade_count):
		...
		var distance = randf_range(48, 160)  # Distance the grenade will travel
		var target_position = global_position + direction * distance

		# 40px random offset to avoid perfect circle origin
		var spawn_offset = direction.rotated(randf_range(-0.3, 0.3)) * randf_range(8, 24)
